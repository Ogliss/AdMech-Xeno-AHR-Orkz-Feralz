	...
	public class IncidentWorker_OrkoidFungalSprout : IncidentWorker
	{
		...
		protected override bool TryExecuteWorker(IncidentParms parms)
		{
			...
			for (int i = 0; i < randomInRange; i++)
			{
				...
				Thing thing2 = GenSpawn.Spawn(OGOrkThingDefOf.Plant_OrkoidCocoon, intVec, map, WipeMode.Vanish);
				if (thing == null)
				{
					thing = thing2;
				}
		...
		{
			if (!c.Standable(map) || c.Fogged(map) || map.fertilityGrid.FertilityAt(c) < OGOrkThingDefOf.Plant_OrkoidCocoon.plant.fertilityMin || !c.GetRoom(map, RegionType.Set_Passable).PsychologicallyOutdoors || c.GetEdifice(map) != null || !PlantUtility.GrowthSeasonNow(c, map, false))
			{
				return false;
			}
			...
			{
				if (thingList[i].def == OGOrkThingDefOf.Plant_OrkoidCocoon)
				{
					return false;
				}
			}
